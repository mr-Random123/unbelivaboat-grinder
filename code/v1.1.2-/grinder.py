...
nversion = []
def update():
    updatebox = messagebox.askquestion(title="update", message="it seems you got the old version press yes to update")
    if updatebox == "yes":
        webbrowser.open_new("https://github.com/mr-Random123/unbelivaboat-grinder/tree/main/exes")
        exit()
def main():
...
if __name__ == '__main__':
    ...
    for letter in f.readline():
        reuse_table1.append(int(letter))

    for letter in reuse_table1:
        reuse_table0.append(letter)
    reuse_table1.pop()
    for letter in reuse_table0:
        nversion.append(letter)

    if version[0] == nversion[1]:

        if version[1] == nversion[1]:

            if version[2] <= nversion[2]:
                update()
        else:
            update()
    else:
